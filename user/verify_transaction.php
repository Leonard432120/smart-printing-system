...
// Step 2: Fetch payment info from your DB
...
$secret = "SEC-TEST-pAOEgDVN5abHFnHkf5HCUkluBK01Pzi6"; // Live in production
...
if (isset($data['data']['status']) && $data['data']['status'] === 'success') {
    ...
    elseif ($order_id) {
        ...
        // ✅ Redirect to success page (order)
        header("Location: payment_success.php?tx_ref=" . urlencode($transaction_id) . "&order_id=" . $order_id);
        exit;
    }

    // Step 8: Fallback if neither
    echo "✅ Payment verified, but no related record found.";
    exit();
